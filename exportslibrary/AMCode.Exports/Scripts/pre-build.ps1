...
if ( $Env.SKIP_DOTNET_FORMAT -eq $null ) {
	if ( $expectedSolutionName -eq $solutionName ) {
		...
		dotnet format $solutionDirectory$projectName.IntegrationTests/$projectName.IntegrationTests.csproj --fix-style warn
		dotnet format $solutionDirectory$projectName.SharedTestLibrary/$projectName.SharedTestLibrary.csproj --fix-style warn
