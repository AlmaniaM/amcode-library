...
/// <summary>
/// Azure OpenAI Service provider for recipe parsing
/// </summary>
public class AzureOpenAIProvider : GenericAIProvider
{
        ...
        : base(logger, httpClientFactory)
    {
        ...
        {
            _logger.LogWarning("Azure OpenAI API key, endpoint, or deployment name not configured");
            _httpClient = null!;
        }
        else
        {
    ...
    public override async Task<ParsedRecipeResult> ParseTextAsync(string text, RecipeParsingOptions options, CancellationToken cancellationToken = default)
    {
        try
        {
            if (_httpClient == null)
            {
                throw new InvalidOperationException("Azure OpenAI client not initialized - check API key, endpoint, and deployment name configuration");
            }
            
            _logger.LogInformation("Parsing recipe text with Azure OpenAI");
            
            var prompt = _promptBuilder.BuildRecipeParsingPrompt(text, options);
            var request = CreateAzureOpenAIRequest(prompt, options);
            ...
            if (!response.IsSuccessStatusCode)
            {
                ...
                throw new AIException($"Azure OpenAI API request failed: {response.StatusCode} - {errorContent}");
            }
            
            var result = await ProcessResponseAsync(response, cancellationToken);
            
            _logger.LogInformation("Azure OpenAI parsing completed successfully. Cost: ${Cost:F6}, Tokens: {Tokens}", 
        ...
        {
            _logger.LogError(ex, "Azure OpenAI parsing failed");
            throw new AIException($"Azure OpenAI parsing failed: {ex.Message}", ex);
        }
    }
        ...
        try
        {
            if (_httpClient == null)
            {
                return new AIProviderHealth
                {
                    ...
                    ErrorMessage = "Azure OpenAI client not initialized - check API key, endpoint, and deployment name configuration"
                };
            }
            ...
            var request = CreateAzureOpenAIRequest("Hello", new RecipeParsingOptions());
            var requestMessage = new HttpRequestMessage(HttpMethod.Post, $"{_config.Endpoint}/openai/deployments/{_config.DeploymentName}/chat/completions?api-version=2024-02-15-preview")
            {
    ...
    public override async Task<decimal> GetCostEstimateAsync(string text, RecipeParsingOptions options)
    {
        ...
        {
            _logger.LogError(ex, "Failed to calculate cost estimate for Azure OpenAI");
            return 0m;
        }
    }
    ...
    {
        return _httpClient != null && !string.IsNullOrEmpty(_config.ApiKey) && !string.IsNullOrEmpty(_config.Endpoint) && !string.IsNullOrEmpty(_config.DeploymentName);
    }
    ...
    protected override async Task<ParsedRecipeResult> ProcessResponseAsync(HttpResponseMessage response, CancellationToken cancellationToken)
    {
        ...
        {
            throw new AIException("Invalid response from Azure OpenAI");
        }
    ...
    private object CreateAzureOpenAIRequest(string prompt, RecipeParsingOptions options)
    {
        return new
        {
            messages = new[]
