...
/// <summary>
/// Anthropic Claude OCR provider implementation using Claude's vision capabilities
/// </summary>
public class AnthropicOCRService : IOCRProvider
{
    private readonly HttpClient? _httpClient;
    private readonly ILogger<AnthropicOCRService> _logger;
    private readonly AnthropicOCRConfiguration _config;
    ...
    /// <summary>
    /// Initializes a new instance of the AnthropicOCRService class
    /// </summary>
    /// <param name="logger">The logger</param>
    /// <param name="httpClientFactory">The HTTP client factory</param>
    /// <param name="config">The Anthropic OCR configuration</param>
    public AnthropicOCRService(
        ILogger<AnthropicOCRService> logger,
        IHttpClientFactory httpClientFactory,
        IOptions<AnthropicOCRConfiguration> config)
    {
        ...
        {
            _logger.LogWarning("Anthropic API key not configured. Provider will be marked as unavailable.");
            _httpClient = null;
        }
        else
        {
            _httpClient = httpClientFactory.CreateClient(nameof(AnthropicOCRService));
    ...
    /// </summary>
    public string ProviderName => "Anthropic Claude OCR";
    ...
    public OCRProviderCapabilities Capabilities => new OCRProviderCapabilities
    {
        ...
        SupportsBoundingBoxes = false, // Claude doesn't provide bounding boxes
        SupportsConfidenceScores = false, // Claude doesn't provide confidence scores
    ...
    public async Task<OCRResult> ProcessImageAsync(Stream imageStream, OCRRequest options, CancellationToken cancellationToken = default)
    {
        if (_httpClient == null)
        {
            throw new InvalidOperationException("Anthropic client is not available. Please check your API key configuration.");
        }
        ...
        try
        {
            _logger.LogInformation("Processing image with Anthropic Claude OCR");
            ...
            for (int attempt = 0; attempt <= _config.MaxRetries; attempt++)
            {
                try
                {
                    if (attempt > 0)
                    {
                        ...
                        _logger.LogInformation("Retrying Anthropic OCR request (attempt {Attempt}/{MaxRetries})", attempt + 1, _config.MaxRetries + 1);
                    }
                ...
                {
                    lastException = ex;
                    _logger.LogWarning(ex, "Anthropic OCR request failed (attempt {Attempt}/{MaxRetries})", attempt + 1, _config.MaxRetries + 1);
            ...
            if (response == null)
            {
                throw lastException ?? new InvalidOperationException("Failed to get response from Anthropic API");
            }
            ...
            if (!response.IsSuccessStatusCode)
            {
                ...
                _logger.LogError("Anthropic OCR API request failed: {StatusCode} - {Error}", response.StatusCode, errorContent);
                throw new OCRException($"Anthropic OCR API request failed: {response.StatusCode} - {errorContent}");
            }
            ...
            {
                throw new OCRException("Invalid response from Anthropic API - no content returned");
            }

            // Extract text from response
            var extractedText = anthropicResponse.Content[0].Text ?? string.Empty;
            var usage = anthropicResponse.Usage;

            // Calculate cost based on token usage
            var cost = (usage.InputTokens * _config.CostPerInputToken) + 
                       (usage.OutputTokens * _config.CostPerOutputToken);
            ...
            var result = new OCRResult
            {
                ...
                Confidence = 0.9, // Claude provides high-quality OCR, use high default confidence
            ...
            _logger.LogInformation("Anthropic Claude OCR completed successfully in {ProcessingTime}ms. Cost: ${Cost:F6}, Tokens: {Tokens}",
                processingTime.TotalMilliseconds, cost, usage.InputTokens + usage.OutputTokens);

            return result;
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Anthropic Claude OCR failed");
            throw new OCRException($"Anthropic Claude OCR processing failed: {ex.Message}", ex);
        }
    }
    ...
    public Task<decimal> GetCostEstimateAsync(long imageSizeBytes, OCRRequest? options = null)
    {
        ...
        // Rough estimate: ~1 token per 4 bytes of image data (base64 encoded)
        // Plus prompt tokens (~50) and response tokens (~500 for OCR text)
        var base64Size = (long)(imageSizeBytes * 1.33); // Base64 encoding overhead
        var estimatedInputTokens = (int)(base64Size / 4) + 50; // Image tokens + prompt tokens
    ...
    {
        // Anthropic Claude is generally very reliable
        return 0.95;
    }
    ...
    {
        // Anthropic Claude provides high-quality OCR results
        return 0.92;
    }

    /// <summary>
    /// Builds the OCR prompt for Claude
    /// </summary>
    private string BuildOCRPrompt(OCRRequest options)
    {
        var prompt = new StringBuilder();
        prompt.AppendLine("Extract all text from this image. Please provide:");
    ...
    private List<TextBlock> ParseTextIntoBlocks(string text, OCRRequest options)
    {
        ...
        for (int i = 0; i < lines.Length; i++)
        {
            ...
            {
                Text = line,
                Confidence = 0.9, // High confidence for Claude OCR
                BoundingBox = BoundingBox.FromCoordinates(0, i * 20, 100, 20), // Placeholder bounding box
                IsHandwritten = false, // Claude doesn't distinguish
                IsPrinted = true,
                Language = options.ExpectedLanguage ?? "en",
                ReadingOrder = i
            };
